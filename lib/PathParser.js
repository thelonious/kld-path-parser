    ...
    parseData(pathData) {
        ...
        while (token.typeis(PathLexeme.EOD) === false) {
            ...
            switch (token.type) {
                case PathLexeme.COMMAND:
                    if (mode === BOP && token.text !== "M" && token.text !== "m") {
                        throw new SyntaxError(`New paths must begin with a moveto command: ${token}`);
                    }
            ...
            for (let i = 0; i < parameterCount; i++) {
                switch (token.type) {
                    case PathLexeme.COMMAND:
                        throw new SyntaxError(`Parameter must be a number: ${token.text}`);
                    ...
                    default:
                        throw new SyntaxError(`PathParser.parseData: unrecognized parameter type: ${token.type}`);
                }
            ...
            if (token === lastToken) {
                throw new SyntaxError(`PathParser.parseData: parser stalled: ${token.text}`);
            }
            else {
                lastToken = token;
            }
